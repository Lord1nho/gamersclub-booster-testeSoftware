...
const testCases = [
    ...
    { mapa: 'de_cbble_classic', expectedURL: 'https://www.esportelandia.com.br/wp-content/uploads/2020/01/posi%C3%A7%C3%B5es-csgo-cobblestone.jpg?ezimgfmt=ng:webp/ngcb2'},
...
describe('Teste de integração', () => {
  test.each(testCases)('integração sendMatchInfo com getMapImage - %s', async (testCase) => {
    ...
    const mockSend = (url, data) =>{
      ...
      expect(url).toBe('url_mock')
      //expect(data).toEqual(gcMatch);
    };
    ...
    const mapName = gcMatch.map.name; // Mapa válido para teste
    const mapImage = getMapImage(mapName);

    expect(typeof mapImage).toBe('string'); // Verifica se a URL da imagem é uma string
